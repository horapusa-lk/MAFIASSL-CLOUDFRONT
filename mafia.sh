...
cat << EOF > /etc/MAFIASSL+CLOUDFRONT/certificate.py
...
x = ".acme.sh/acme.sh --register-account -m "
y = x +  Email

#Dname

domainname = input("Whats Your Domain Name=")
 
t = ".acme.sh/acme.sh --issue -d "
u = " --standalone"
m = t + domainname + u



o = ".acme.sh/acme.sh --installcert -d "
p = " --key-file /etc/MAFIASSL+CLOUDFRONT/private.key --fullchain-file /etc/MAFIASSL+CLOUDFRONT/cert.crt"
...
with open('certificate.sh', 'w') as f:
    print(g, file=f)
...
chmod 777 certificate.sh

./certificate.sh

clear

cat private.key cert.crt > stunnel.pem

rm certificate.sh certificate.py
